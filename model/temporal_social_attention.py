...
class TemporalSocialAttentionLayer(torch.nn.Module):
  """
  TGAT layer for social space given the node itself,
   and its neighbors.
  """
  ...
  def __init__(self, n_node_features, n_neighbors_features, time_dim,
               ...
               dropout=0.1):
    super(TemporalSocialAttentionLayer, self).__init__()
    ...
    self.query_dim = n_node_features + time_dim #q (t) = hi^(l-1) (t) ||  φ(0)  ##Eq7 

    self.key_dim = n_neighbors_features + time_dim  # K = V = C^(l)(t) = h1^(l-1) (t) ||  ei1 (t1) || φ(t - t1) || \
    #... hN^(l-1) (t) || φ(t - tN) ##Eq8,9 
